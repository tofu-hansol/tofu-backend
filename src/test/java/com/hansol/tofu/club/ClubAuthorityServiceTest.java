...
class ClubAuthorityServiceTest {
	...
	void setUp() {
		...
		sut = new ClubAuthorityService(memberService, clubService, clubMemberRepository);
	}
	...
	void requestJoinClub_동호회가_존재하는_경우_가입신청이_완료된다() {
		...
		when(clubService.findClubBy(clubId, ClubStatus.ACTIVATE)).thenReturn(Optional.of(clubEntity));
		when(memberService.findMemberBy(1L)).thenReturn(Optional.of(memberEntity));
